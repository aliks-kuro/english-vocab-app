...
| # | タイトル | 概要 |
|---|---------|------|
| 1 | **アナグラム** | 並び替えられた文字タイルをドラッグして元の単語に戻す |
| 2 | **レールゲーム** | トロッコが走るレール上でA〜Dの選択肢から正しい意味を選ぶ |
| 3 | **ダンジョン・タイピング** | ダンジョンRPF風。モンスターの意味を見て英単語をタイピングして撃破 |

#### Game 1 — アナグラム
- 物理演算風の文字タイル（64px 円形ボタン）をドラッグ操作
- 3ミス or タイムアップでゲームオーバー
- 正解で100点

#### Game 2 — レールゲーム（トロッコ）
- 3Dパース視点のレール + TPSスタイルのトロッコ
- ← → キーまたは選択肢ボタンでレーンを選択、Enter/決定で回答
- **走行アニメーション**: 回答確定後にトロッコが約2秒走り続け、正解レールは緑・不正解レールは赤の落とし穴が出現
- タイムアウト時は「今いるレーン」を自動選択（無回答なし）
- 画面下部に間違えた単語リストを表示

#### Game 3 — ダンジョン・タイピング
- Lv.1（スライム）〜 Lv.7（魔王）の7段階難易度
- 単語の長さで難易度を自動分類（4文字以下→Lv.1、13文字以上→Lv.7）
